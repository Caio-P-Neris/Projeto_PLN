...
# Neste trabalho, implementamos uma ferramenta de análisa do mercado financeiro por meio de notícias (usando a NewsAPI), possibilitando:
...
*   sua comparação com a economia de outros países,
*   de empresas, setores e ações específicas  

2.   Classificação de relevância e similaridade das notícias para cenário econômico, extraindo palavras-chaves

3. Sumarização do cenário econômico
...
def analise_pais(pais, noticias: pd.DataFrame):
    ...
    try:
        ...
        # Exibe resumo com melhor formatação
        print(f"\n📰 **Resumo do cenário econômico ({pais}):**\n")
        print(resumo.strip())
...
acoes = ['VALE3', 'Petrobras', 'Banco Itaú SA', 'Apple'] #, 'BRFS3.SA', 'ABEV3.SA', 'WEGE3.SA',
